...
The `_reversa_sdd/` directory contains generated specifications derived from Bevy analysis. Those documents include useful architectural patterns, but also describe systems far beyond the `seishin` MVP.
...
- Conflicting `_reversa_sdd` signals are resolved in favor of the explicit `seishin` MVP contract.
